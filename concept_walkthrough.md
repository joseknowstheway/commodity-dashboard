...
### What was built
- Expanded `README.md`: a real dashboard **screenshot**, a project-structure
  tree, a **Design Decisions** section (repository pattern, SQLite vs Postgres,
  raw vs processed, ARIMA, EIA, Dash), and a **Development** section.
- A reproducible `scripts/capture_screenshot.py` (headless Chromium via
  Playwright) that renders the live app to `assets/dashboard.png`.
- A docstring + type-hint audit: every public class/function is documented
  (Google-style) and annotated.
- Final `black`/`flake8` pass (clean across 20 files) and the 32-test suite green.
- Tagged **`v1.0.0`**.

### Key ideas

**1. The README is the project's front door.**
A hiring manager reads the README before any code. So it leads with a real
screenshot, states what/why up front, and includes a Design Decisions section
that pre-answers "why did you build it this way?" — turning the repo into a
conversation starter rather than a code dump.

**2. Document the *why*, not the *what*.**
The code already says what it does; the README explains the trade-offs the code
can't — why SQLite over Postgres, why ARIMA(1,1,1), why split raw from processed.
That's the signal of an engineer who makes deliberate choices.

**3. A reproducible screenshot.**
Rather than a hand-grabbed image, the screenshot is generated by a committed
script: start the app headless, wait for the Plotly SVGs, capture full-page. It
documents itself and can be regenerated whenever the UI changes.

**4. Consistent docstrings and type hints throughout.**
Google-style docstrings on every public surface and type hints on every
signature make the code self-describing and IDE-friendly, and they're what static
analysis / API docs tooling would consume.

**5. Tooling as a quality gate.**
`black` (formatting) and `flake8` (linting) pass cleanly, and the suite runs in
~1.3s. These are exactly the gates a CI pipeline would enforce on every push.

**6. Semantic versioning.**
Tagging `v1.0.0` marks the first complete, working release — a clear, citable
milestone in the history.

### Mistakes & fixes
- **Screenshot tooling wasn't installed.** No kaleido/Playwright/Selenium were
  present. **Fix:** installed Playwright + headless Chromium and scripted the
  capture, producing a faithful full-page image instead of a placeholder.
- **A handful of `__init__`/dunder methods lacked their own docstrings.** Their
  args were documented on the class, but the audit flagged them. **Fix:** added
  concise docstrings so the coverage check is clean.

### Interview Q&A

**Q: Walk me through this project.**
A: It's an end-to-end commodity-price analytics app. An OOP client pulls weekly
energy prices from the EIA v2 API with retries and backoff; a pure-function
pipeline cleans and enriches them (rolling stats, z-scores, robust outlier
flags); a repository persists raw and derived tables to SQLite idempotently; an
orchestration CLI wires it together with incremental loading; an ARIMA model adds
forecasts with confidence intervals; and a Plotly Dash dashboard visualizes it
all. It's covered by 32 tests and formatted/linted clean.

**Q: What are you most proud of, technically?**
A: The separation of concerns. Every layer has one job and a clean contract, so
each is independently testable — the dashboard logic is tested without a browser,
the DB logic against an in-memory database, the pipeline as pure functions. That
discipline is also what let me catch real bugs, like the global-vs-local outlier
detector.

**Q: If you had more time, what would you add?**
A: A GitHub Actions workflow to run the suite on every push, deployment to
AWS (the app is already WSGI-ready and the repository abstraction makes RDS
Postgres a drop-in), and alerting when a z-score breaches ±2.

**Q: How would you scale this?**
A: Replace the polling CLI with event-driven ingestion (e.g. Lambda +
EventBridge on a schedule), promote SQLite to RDS Postgres behind the same
repository interface, and run the dashboard behind gunicorn on EC2/Elastic
Beanstalk. None of that touches the pipeline or dashboard code, thanks to the
layering.
